...
if [ "$PROJECT_TYPE" == "integration" ]; then
  ...
  case "$OSTYPE" in
    darwin*)
      KKA_LB_INT_HTTP_PORT=$(kubectl get service -n istio-ingress istio-ingressgateway -o go-template='{{range .spec.ports}}{{if (eq .port 80)}}{{.nodePort}}{{end}}{{end}}')
    ...
    docker run \
      ...
      --network kind \
      alpine/socat -dd tcp-listen:$KKA_LB_INT_HTTP_PORT,fork,reuseaddr tcp-connect:target:$KKA_LB_INT_HTTP_PORT || true
